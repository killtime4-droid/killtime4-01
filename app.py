...
# 1. 앱 기본 설정 (글자 크기가 비대해지지 않도록 본문 크기로 통일)
...
# 🌟 발급받으신 진짜 API 키 고정
CRITICAL_API_KEY = "AIzaSyCObm0Vibi_-2gOdqSt-izu-CE4aNWFUFs"
...
# 3. 사진이 업로드되면 버튼 클릭 없이 '즉시 자동 분석' 진행
if uploaded_file is not None:
    ...
    with st.status("🚀 구글 AI가 질환별 핵심 지표를 정밀 분석 중입니다...", expanded=True) as status:
        try:
            ...
            # [구분 라인 지침] 각 PART 시작 직전에 마크다운 구분선(---)을 넣도록 프롬프트 조절
            prompt = """
            ...
            5. 세부 설명 내용(보조글씨)은 꾸밈없는 일반 텍스트로만 정갈하게 작성하세요.
            ...
            ---
            :red[**PART 5. 🚨 종합 위험도 판정 및 조리 대안**]
            - :blue[종합 위험도 평가]: 일상 섭취 시 내 몸에 미치는 해로움을 분석하여 [안전 / 주의 / 위험] 등급 중 하나로 진단하세요.
            - :blue[건강한 조리 대안 팁]: 면을 따로 삶아 기름기를 버리는 법, 스프 조절, 칼륨이 풍부한 채소(파, 양파 등)를 추가해 나트륨 배출을 돕는 방법을 상세히 안내하세요.
            """
            
            response = model.generate_content(contents=[prompt, image])
            
            status.update(label="🎉 파트별 구분선 리포트 생성 완료!", state="complete", expanded=False)
            ...
            st.error(f"에러 원인: {e}")
            st.info("현재 구글 AI 일일 호출 한도(20회)가 초과된 상태입니다. 내일 오후에 다시 실행하시면 정상 작동합니다.")
